...
class CacheManager:
    ...
    async def read_file(self, full_fs_path, offset, length, mtime):
        ...
        async with trio.open_nursery() as nursery:
            for i in range(start_chunk, end_chunk + 1):
                nursery.start_soon(
                    self._fetch_chunk_task, file_id, api_path, i, chunks_map
                )
        ...
        for i in range(start_chunk, end_chunk + 1):
            if i not in chunks_map:
                raise IOError(f"Chunk {i} missing")
    ...
    async def _fetch_chunk_task(self, file_id, api_path, chunk_idx, out_map):
        ...
        data = await trio.to_thread.run_sync(
            self.disk.retrieve_chunk, file_id, chunk_idx
        )
        if data:
        ...
        # 2. Network
        data = await self.uc_client.download_chunk(
            api_path, chunk_idx * self.chunk_size, self.chunk_size
        )
        if not data:
            data = b""
