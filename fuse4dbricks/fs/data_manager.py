...
class DataManager:
    ...
    async def _read_chunk(self, fs_path: str, chunk_id: int, mtime: float, chunk_size: int, out_dict):
        ...
        # 2. Coalescing
        wait_event = await join_or_lead_request(self._inflight_lock, self._inflight_downloads, (fs_path, chunk_id, mtime))
        ...
        try:
            ...
            await notify_followers(self._inflight_lock, self._inflight_downloads, (fs_path, chunk_id, mtime))
        # save chunk in out_dict
        out_dict[chunk_id] = chunk

    async def read(self, fs_path: str, offset: int, length: int, mtime: float, file_size: int) -> bytes:
        """
        Reads file contents
        ...
        async with trio.open_nursery() as nursery:
            for chunk_id in chunks_to_read:
                ...
                nursery.start_soon(self._read_chunk, fs_path, chunk_id, mtime, chunk_size, chunks)
        # Assemble chunks
        result = bytearray()
        for chunk_id in chunks_to_read:
