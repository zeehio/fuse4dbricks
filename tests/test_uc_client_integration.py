...
async def test_live_404_handling(live_client):
    ...
    try:
        ...
        meta = await live_client.get_file_metadata("/Volumes/main/default/non_existent/ghost.txt")
        assert meta is None
        
        # Lista de carpeta fantasma -> []
        contents = await live_client.list_directory_contents("/Volumes/main/default/non_existent_folder")
        assert contents == []
    finally:
        await live_client.close()
